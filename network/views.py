...
def view_profile(request, id):
    ...
    return render(request, 'network/profile.html',
                  {'posts': page_posts, 'paginator': p, 'userx': requested_user, 'followers': fwers,
                   'following': following})
...
def favourites(request):
    ...
    posts_query_set = Post.objects.filter(posted_by__in=users).order_by('-created')
...
def edit(request, post_id):
    if request.method == "POST":
        ...
        if request.user.username != str(post.posted_by):
    ...
    if request.method == "PUT":
        ...
        data = json.loads(request.body)
        user = request.user
        action = data['whatToDo']
        id = data['id']

        if action == "add":
            p = Post.objects.get(pk=id)
            p.likes.add(user)
            return JsonResponse({'Done': True, "Updated": len(p.likes.all())})
        ...
        return JsonResponse({'Done': False, "Updated": 155})

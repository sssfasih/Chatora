...
def bid(request,p_id):
    if request.method == 'POST':
        ...
        try:
            ...
            return HttpResponse("Enter bid in valid format")
        product = AuctionProduct.objects.get(pk=p_id)
        user = request.user
        if product.highest < amount:
            product.highest = amount
            product.winner = user
            product.save()
        else:
            return HttpResponse(f"Please enter bid higher than current price (i.e{product.highest})")
    return HttpResponseRedirect(reverse('listing',args={p_id}))
...
def category(request,cat_name):
    cat_obj = ProductCategory.objects.get(category=cat_name)
    cat_id= cat_obj.id
    prods = AuctionProduct.objects.filter(cat_id=cat_id)
